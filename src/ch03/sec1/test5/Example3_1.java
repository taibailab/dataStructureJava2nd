...
import ch03.sec1.test3.SqStack;

/**
 * 3.1.栈
 * 3.1.5.栈的应用
 * ->[例3.1].分隔符匹配问题：编写判断Java程序中分隔符是否匹配的程序。Java程序中有以下分隔符：圆括号"（"和"）"，方括号"["和"]"，大括号"{"和"}"以及注释分隔符"/*"
 * 和"* /"
 */
public class Example3_1 {
	private static final int LEFT = 0;	// 记录分隔符为"左"的分隔符
	...
	public int verifyFlag(String str) {
		if ("(".equals(str) || "[".equals(str) || "{".equals(str) || "/*".equals(str)) 	// 左分隔符
			return LEFT;
		else if (")".equals(str) || "]".equals(str) || "}".equals(str) || "*/".equals(str)) 	// 右分隔符
			return RIGHT;
		else 
			// 其他字符
			return OTHER;
		...
		if (!"".equals(str) && str != null) {
			...
			for (int i = 0; i < length; i++) {
				...
				if (i != length) {	// c不是最后一个字符
					if (('/' == c && '*' == str.charAt(i + 1)) || ('*' == c && '/' == str.charAt(i + 1))) {	// 是分解符"/*"或"*/"
