...
public class DuLinkList implements IList {
	...
	public Object get(int i) throws Exception {
		...
		if (j > i || p.equals(head)) {	// i小于0或者大于等于表长减1
	...
	// 其中i取值范围为：0≤i≤length()。当i=0时表示在表头插入一个数据元素，当i=length()时表示在表尾插入一个数据元素x。
	@Override
	...
	// 将双向循环链表中第i个数据元素删除。其中i取值范围为：0≤i≤ength()-1。
	@Override
	...
	public int indexOf(Object x) {
		DuLNode p = head.next;	// 初始化，p指向首结点，j为计数器
		int j = 0;
		while (!p.equals(head) && !p.data.equals(x)) {	// 从链表中的首结点元素开始查找，直到p.data指向元素x或到达链表的尾端
			p = p.next;	// 指向下一个元素
			++j;	// 计数器的值增1
		}
	...
	@Override
	public void display() {
		DuLNode node = head.next;	// 取出带头结点的双向链表的首结点
