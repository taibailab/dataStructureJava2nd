...
public class SqList implements IList {
	...
	// 在线性表的第i个数据元素之前插入一个值为x的数据元素。其中i取值范围为：0≤i≤length()。如果i值不在此范围内则抛出异常，当i=0时表示在表头插入以数据元素x，当
	...
	// 将线性表中第i个数据元素删除。其中i取值范围为：0≤i≤length()- 1，如果i值不在此范围内则抛出异常
	@Override
	public void remove(int i) throws Exception {
		if (i < 0 || i > curLen - 1) 	// i小于1或者长度大于表长减1
	...
	public int indexOf(Object x) {
		...
		while (j < curLen && !listElem[j].equals(x)) 
			// 从顺序表中的首结点开始查找，知道listElem[j]指向元素x或到达顺序表的表尾
			j++;
